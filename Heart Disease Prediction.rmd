---
title: "Untitled"
author: "Krittapas Suwansiri 6580233026"
date: "2023-04-13"
...
```
Converting binary variables to characters is done for better visualization purposes. By default, R treats binary variables as numeric with 0s and 1s. This can make it difficult to read and interpret the data when we want to look at the actual categories (e.g., male/female or smoker/non-smoker). By converting the binary variables to characters, we can clearly see the categories and their labels in the output. This can be particularly useful when we want to create plots or tables to summarize the data.
...
the plot_grid() function from the cowplot package is used to combine the two plots into one grid with a title on top.
...
############################################################################################################################
Other conclusion for specific group

Although the accuracy of the model is 75%-80%, it is not effectively identifying individuals with a higher risk of heart disease. Our primary interest is in predicting those individuals who are at greater risk, as this will allow for earlier treatment. Therefore, we will attempt to adjust the threshold and calculate the accuracy specifically for individuals with a 10-year risk of coronary heart disease.
```{r}
pred_test_data_lowthreshold <- ifelse(pred_model>0.1,1,0)
...
paste("Accuracy for those who have 10 year risk of coronary heart disease with threshold 0.1 is",acc_for_1,"%")
```
The accuracy for individuals who have a 10-year risk of coronary heart disease with a threshold of 0.1 is 80.12. This indicates that our model is capable of accurately predicting individuals who are at risk of heart disease, allowing us to provide early treatment and guidance.
